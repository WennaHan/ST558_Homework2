...
sum_fun <- function (data_list, stat = "mean"){
  my_fun <- get(stat)
  my_sum <- c(my_fun(data_list$treatment$pre_bp), 
              my_fun(data_list$treatment$post_bp),
              my_fun(data_list$treatment$diff_bp),
              my_fun(data_list$placebo$pre_bp), 
              my_fun(data_list$placebo$post_bp),
              my_fun(data_list$placebo$diff_bp))
  my_name <- vector()
  my_name[1:3] <- paste(stat, "treatment", c("pre", "post","diff"), sep = "_")
  my_name[4:6] <- paste(stat, "placebo", c("pre", "post","diff"), sep = "_")
  names(my_sum) <- my_name
  my_sum
}
```
